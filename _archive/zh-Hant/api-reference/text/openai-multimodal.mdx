...
<ParamField body="model" type="string" required>
  ...
  * `gpt-5` - GPT-5 基礎模型
  * `gpt-5-pro` - GPT-5 專業增強版（僅支援此介面）
  * `gpt-5-codex` - GPT-5 Codex 程式碼模型（僅支援此介面）
  * `gpt-5.1-codex` - GPT-5.1 Codex 程式碼模型（僅支援此介面）
  * `gpt-5.1-codex-mini` - GPT-5.1 Codex Mini（僅支援此介面）
  * `gpt-5.2-codex` - GPT-5.2 Codex 程式碼模型（僅支援此介面）
  * `gpt-5.3-codex` - GPT-5.3 Codex 程式碼模型（僅支援此介面）
  * 以及所有支援 `/v1/chat/completions` 的模型

  <Note>
  `gpt-5-pro` 和所有 Codex 系列模型僅支援此介面（`/v1/responses`），不支援 `/v1/chat/completions`。
  </Note>
</ParamField>
