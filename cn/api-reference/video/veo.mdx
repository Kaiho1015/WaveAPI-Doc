---
title: "Veo 系列"
description: "Google Veo 3.1 — 官方直连 Fast / Quality 双档(按秒、原生有声),逆向 Fast / Lite / Quality 三档(按次固定价)"
api: "POST https://www.qingbo.dev/v1/tasks"
---

Google Veo 3.1 视频生成模型,提供 **官方直连** 与 **逆向接入** 两种渠道,共 **5 个子档**。所有子档时长均**固定 8 秒**,支持 720P / 1080P / 4K 三种分辨率。

- **官方档**(`-official`):**按秒计费**(单价/秒 × 时长),原生支持音频(`generate_audio=true` 触发 `_sound` 加价档)。
- **逆向档**(无 `-official`):**按次计费**(单次固定价,**与时长无关**),不支持音频。

| 档位 | 渠道 | 计费方式 | 音频 | 定位 |
|---|---|---|---|---|
| `veo3.1-quality-official` | 官方直连 | 按秒 | 支持 | 旗舰档,最终交付质量 |
| `veo3.1-fast-official` | 官方直连 | 按秒 | 支持 | 加速档,适合迭代试稿 |
| `veo3.1-quality` | 逆向接入 | 按次 | 不支持 | 逆向旗舰子型 |
| `veo3.1-fast` | 逆向接入 | 按次 | 不支持 | 逆向加速子型 |
| `veo3.1-lite` | 逆向接入 | 按次 | 不支持 | 逆向轻量子型,低成本批量 |

## 定价

<Note>
两类渠道计费口径**完全不同**:官方档 **按秒**(下表为每秒单价,需乘以时长 8 秒),逆向档 **按次**(下表即单次固定价,不乘时长)。
</Note>
...
| `veo3.1-quality` | 逆向接入 | 按次 | 8 秒(固定) | 720P / 1080P / 4K | 不支持 |
| `veo3.1-fast` | 逆向接入 | 按次 | 8 秒(固定) | 720P / 1080P / 4K | 不支持 |
| `veo3.1-lite` | 逆向接入 | 按次 | 8 秒(固定) | 720P / 1080P / 4K | 不支持 |
...
<Note>
官方档与逆向档参数有差:**仅官方档**支持 `generate_audio` / `enhance_prompt` / `person_generation` / `sample_count` / `resize_mode`;逆向档参数更精简,且**不支持音频**。
</Note>
...
<Tabs>
  <Tab title="veo3.1-quality-official / veo3.1-fast-official(官方)">
    ...
    <ParamField body="enhance_prompt" type="boolean" default="true">
      上游提示词增强,**只能为 `true`**(设 `false` 会报错;不需要时直接不传)
    </ParamField>
  ...
  <Tab title="veo3.1-quality / veo3.1-fast(逆向)">
    ...
    <ParamField body="official_fallback" type="boolean" default="false">
      逆向失败时是否回退官方渠道。`true` 启用,`false` 不启用(默认)
    </ParamField>

    <Note>
    逆向档 **按次计费**(固定 8 秒,与时长无关),**不支持** `generate_audio`。`image_urls` 最多 3 张。
    </Note>
  </Tab>

  <Tab title="veo3.1-lite(逆向·轻量)">
    <ParamField body="enable_gif" type="boolean" default="false">
      输出 GIF 格式。注意:GIF 与 1080p / 4k 分辨率**不可同时使用**
    </ParamField>
...
| 项目 | 限制 |
|---|---|
| 参考图片 | JPG / PNG / WEBP,逆向档单张 ≤ 10MB、最多 3 张 |
| 输出视频 | MP4,链接 24 小时有效 |
| 时长 | 全档固定 8 秒 |
| 分辨率 | 720P / 1080P / 4K 全档支持 |
| 音频 | 仅官方档(`-official`)支持 `generate_audio`,逆向档不支持 |
| GIF | 仅逆向档 `enable_gif`,且不可与 1080p / 4k 同用 |
| 人物 | 官方档受 `person_generation` 策略约束,违规内容会被上游拦截 |
